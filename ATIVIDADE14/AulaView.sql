...
create view vw_aniversariantes_mes2 as 
select nome_cliente, telefone_cliente, day(data_nascimento) as dia_aniversario
from tb_cliente
where month(data_nascimento) = month(curdate());

select * from vw_aniversariantes_mes2
where dia_aniversario = 30;

drop view vw_aniversariantes_mes;
...
create view vw_aniversariante_por_mes as
...
from vw_aniversariante_por_mes;

-- Atividade 1: Criar uma view (vw_cliente_info) com o nome, telefone e somente o ano de nascimento dos clientes.
CREATE VIEW vw_cliente_info AS
SELECT nome_cliente, telefone_cliente, data_nascimento, 
YEAR(data_nascimento) AS ano_nascimento
FROM tb_cliente;

-- Atividade 2: Criar uma view (vw_produtos_estoque_alto) para listar o nome e preço de venda dos produtos com estoque acima de 100.
CREATE VIEW vw_produtos_estoque_alto AS
SELECT nome_produto, preco_venda
FROM tb_produto
WHERE estoque > 100;

-- Atividade 3: Criar uma view (vw_cliente_idade) para exibir a idade dos clientes calculada pela diferença entre o ano atual e o ano de nascimento.
CREATE VIEW vw_cliente_idade AS
SELECT nome_cliente, YEAR(CURDATE()) - YEAR(data_nascimento) AS idade
FROM tb_cliente;

-- Atividade 4: Criar uma view (vw_produtos_higiene) que exibe produtos de higiene e limpeza, com uma coluna indicando se o estoque é baixo (estoque < 50) ou alto (estoque >= 50).
CREATE VIEW vw_produtos_higiene AS
...
FROM tb_produto
WHERE categoria = 'Higiene e Limpeza';

-- Atividade 5: Criar uma view (vw_cliente_sp) que exibe o nome e a cidade dos clientes que moram no estado de São Paulo (SP).
CREATE VIEW vw_cliente_sp AS
SELECT nome_cliente, cidade AS cidade
FROM tb_cliente
WHERE estado = 'SP';

-- Atividade 6: Utilizar a view vw_cliente_info para selecionar clientes que nasceram antes de 1980.
SELECT nome_cliente, telefone_cliente, ano_nascimento
FROM vw_cliente_info
WHERE ano_nascimento < 1980;

-- Atividade 7: Usar a view vw_produtos_estoque_alto para listar os produtos com preço de venda superior a 5.00.
SELECT nome_produto, preco_venda
FROM vw_produtos_estoque_alto
WHERE preco_venda > 5.00;

-- Atividade 8: Utilizar a view vw_cliente_idade para listar clientes com idade superior a 30 anos.
SELECT nome_cliente, idade
FROM vw_cliente_idade
WHERE idade > 30;

-- Atividade 9: Selecionar produtos de higiene com estoque alto usando a view vw_produtos_higiene.
SELECT nome_produto, estoque, situacao_estoque
FROM vw_produtos_higiene
WHERE situacao_estoque = 'Alto';

-- Atividade 10: Usar a view vw_cliente_sp para exibir os nomes dos clientes de São Paulo, ordenados pela cidade.
